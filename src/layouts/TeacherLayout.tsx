...
export const TeacherLayout: React.FC = () => {
  ...
  const navItems = [
    ...
    { label: 'Attendance Entry', href: '/teacher/attendance', icon: <UserCheck className="w-5 h-5" /> },
    { label: 'Results & Marks', href: '/teacher/results', icon: <FileSpreadsheet className="w-5 h-5" /> },
    { label: 'Students Roster', href: '/teacher/students', icon: <Users className="w-5 h-5" /> },
  ];
  ...
  const displayName = user ? `${user.firstName} ${user.lastName}`.trim() : 'Faculty Account';
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex text-slate-900 dark:text-slate-100">
      <aside className="hidden lg:flex flex-col w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 shrink-0">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800">
          <Link to="/" className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-sky-600 dark:bg-sky-500 flex items-center justify-center text-white font-bold">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                  isActive
                    ? 'bg-sky-50 dark:bg-sky-950/70 text-sky-600 dark:text-sky-400'
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 flex items-center justify-between">
          ...
          <span className="font-bold text-sm text-slate-600 dark:text-slate-300">Teacher Portal</span>
