...
export const StudentLayout: React.FC = () => {
  ...
  const navItems = [
    ...
    { label: 'My Attendance', href: '/student/attendance', icon: <UserCheck className="w-5 h-5" /> },
    { label: 'My Results', href: '/student/results', icon: <FileSpreadsheet className="w-5 h-5" /> },
    { label: 'My Fees', href: '/student/fees', icon: <DollarSign className="w-5 h-5" /> },
    { label: 'Events', href: '/student/events', icon: <Calendar className="w-5 h-5" /> },
    { label: 'Notices', href: '/student/notices', icon: <FileText className="w-5 h-5" /> },
  ...
  const displayName = user ? `${user.firstName} ${user.lastName}`.trim() : 'Student Portal';
  const displayEmail = user?.email || 'Awaiting API authentication...';
        ...
        <header className="sticky top-0 z-30 h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 sm:px-6 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="relative hidden sm:block w-64 md:w-80">
              ...
              <input
                type="text"
                placeholder="Search portal records, results, fees..."
