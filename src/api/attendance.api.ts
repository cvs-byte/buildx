...
import { extractIdentifierFromQR } from '../utils/qrParser';
...
export const attendanceApi = {
  ...
  async validateStudentQRScan(params: {
    ...
    // 1. Decode a lookup identifier (email OR canonical student/user ID) from the QR payload.
    //    QR passes generated by this app (StudentQRCardModal / UserQRModal) encode the user's
    //    canonical ID, not necessarily an email — so both forms must be accepted here.
    const identifier = extractIdentifierFromQR(params.rawQR);
    console.log('[QR DECODED IDENTIFIER]:', identifier);

    if (!identifier || !identifier.value) {
      console.warn('[QR SCAN REJECTED] Not a recognizable student identifier:', params.rawQR);
      return {
        success: false,
        status: 'INVALID_TOKEN',
        message: 'INVALID QR\nThis QR code is not linked to a registered student.',
      };
    }

    // 2. Lookup student by email or canonical ID in the Users API (GET /Users)
    let student: User | null = null;
    try {
      student = await userApi.findUserByEmail(identifier.value);
    } catch (err: any) {
    ...
    if (!student) {
      console.warn(`[STUDENT LOOKUP FAILED] No user found for identifier: ${identifier.value}`);
      return {
        success: false,
